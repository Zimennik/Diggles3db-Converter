...
def safely_remove_directory(dir_path):
    ...
    try:
        # Сначала удаляем все файлы в директории
        for root, dirs, files in os.walk(dir_path, topdown=False):
            for file in files:
                file_path = os.path.join(root, file)
                try:
                    os.chmod(file_path, 0o777)  # Пытаемся изменить права доступа
                    os.remove(file_path)
...
Examples:
  ...
  python run_with_mapping.py assets/models/hamster.3db
    ...
    # Directories to clean up - НЕ удаляем gltf!
    dirs_to_cleanup = [
        os.path.join("exports", "fbx", f"{model_name}_textures"),
        os.path.join("exports", "fbx", "textures"),  # Папка текстур в fbx
        os.path.join("textures")  # Корневая директория текстур, если создана
    ]
    
    # Найдем дополнительно все директории с 'textures' в названии в fbx директории
    # НЕ трогаем gltf директорию!
    for root, dirs, files in os.walk(os.path.join("exports", "fbx")):
        for dirname in dirs:
            if "textures" in dirname.lower() and os.path.join(root, dirname) not in dirs_to_cleanup:
                # Исключаем только .fbm директории, которые нужно сохранить
