...
def export_to_fbx_binary(model: Model, output_path: str = 'exports/fbx/model.fbx') -> None:
    ...
    # Проверяем, доступен ли Blender перед началом экспорта
    blender_path = None
    
    # Сначала проверяем путь из конфигурационного файла
    ...
    # Если путь в конфигурации не указан или недействителен, ищем Blender в стандартных местах
    if not blender_path:
